...
const renderFeed = (content) => {
  ...
  for (let i = 0; i < feeds.length; i += 1) {
    ...
    if (i === 0) {
      ...
      if (!watchedState.uiState.posts.includes({
        title: feeds[i].title.textContent, readed: false,
      })) {
      ...
      boxEl.append(linkEl);
      boxEl.append(modalButton(`postModal-${15}-${i}`));
      boxEl.append(modalWindow(`postModal-${15}-${i}`, feeds[i].title.innerHTML, feeds[i].description.innerHTML, feeds[i].link));
